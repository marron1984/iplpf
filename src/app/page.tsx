      ...
      >
        <div className="max-w-6xl mx-auto px-4 h-16 md:h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className={`font-bold text-sm md:text-base transition-colors ${scrolled ? 'text-slate-900' : 'text-white'}`}>
              国際P-LP財団
            </span>
          </Link>
            ...
            {NAV_ITEMS.map((item) => (
              <Link
                ...
                className={`text-sm font-medium transition-colors ${
                  scrolled ? 'text-slate-600 hover:text-[#0052A4]' : 'text-white/80 hover:text-white'
                }`}
              >
                {item.label}
              </Link>
      ...
      <main>
        ...
        <section className="py-12 bg-white border-y border-slate-100">
          <div className="max-w-6xl mx-auto px-4 text-center">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-6">パートナー</p>
            <img
              src={PARTNER_NISSAN_URL}
              alt="日産自動車"
              className="h-10 md:h-14 w-auto mx-auto object-contain opacity-50 hover:opacity-100 transition-opacity"
            />
          </div>
        </section>
